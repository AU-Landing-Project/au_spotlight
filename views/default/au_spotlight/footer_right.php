...
$text = '';
$default_text = elgg_get_plugin_setting('auspotlight_generic_tr_loggedin', PLUGIN_ID);
if ($default_text != "" && elgg_is_logged_in()) {
	$text = $default_text;
} else {
	$text = elgg_get_plugin_setting('auspotlight_generic_tr', PLUGIN_ID);
}
...
if ($thiscontext !== false) {
	...
	$ts = array();
	$ts_str = elgg_get_plugin_setting('auspotlight_context_tr', PLUGIN_ID);
	if ($ts_str) {
	...
	if ((elgg_is_logged_in() && isset($loggedinonly[$thiscontext])) || !$loggedinonly[$thiscontext]) {
